...
zoop.db = function( DS, p ) {
 ...
 # data view of zooplakton from biochem db
  ...
  zoop.dir = project.datadirectory("aegis", "biochem", "zooplankton")
  zoop.data.dir = file.path( zoop.dir, "data" )
  zoop.datadump.dir = file.path( zoop.dir, "data", "datadump" )

  if ( DS == "zoop.data.dump.rawdata" ) {
    if (FALSE) {
...
, m.name
    ...
    dir.create( zoop.datadump.dir, recursive=TRUE, showWarnings=FALSE )
...
and to_number(to_char (m.start_date,'YYYY')) =",  YR )
      ...
      fn = file.path( zoop.datadump.dir, paste( "zoop.data.dump.rawdata", YR, ".rdata", sep="")  )
      res = NULL
  ...
  if ( DS == "zoop.data.rawdata.all.redo" ) {
    fn.all = file.path( zoop.datadump.dir, paste( "zoop.data.dump.rawdata", "all_years", "rdata", sep=".")  )
    out = NULL
    for ( YR  in p$zoop.years ) {
      fn = file.path( zoop.datadump.dir, paste( "zoop.data.dump.rawdata", YR, ".rdata", sep="")  )
      res = NULL
  ...
  if ( DS %in% c("zoop.qa.qc", "zoop.qa.qc.redo")  ) {
    fn = file.path( zoop.data.dir, paste( "zoop.data.qa.qc", ".rdata", sep="") )
    if ( DS == "zoop.qa.qc" ) {
      df = NULL
      if (file.exists(fn)) load(fn)
      return(df)
    ...
    zoo = zoop.db( DS="zoop.data.rawdata.all", p=p )
  ...
  if ( DS %in% c("zoop.totalSamples", "zoop.speciesAbund",
    ...
    fn1 = file.path( zoop.data.dir, paste( "zoop.totalSamples", ".rdata", sep="") )
    fn2 = file.path( zoop.data.dir, paste( "zoop.speciesAbund", ".rdata", sep="") )
  ...
  if (DS=="plotdata") {
    ...
    plm = xyplot( header_start_lat ~ header_start_lon| factor(month,labels=month.name),
                  ...
                  panel = function(x, y, subscripts, ...) {
                    coast=  maps::map( "worldHires", regions=c("Canada", "USA"),
